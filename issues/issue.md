...
| I-013 | Sprint 1 | MAJOR | FTF API `status` field may not match CRM status — staging mismatch confirmed | dev_manager | OPEN | 2026-05-22 | Order 1000276072: API returns `status="Quote"`, CRM shows "Checking". Prateek has raised this with FTF developer to verify. Need confirmation: (1) does `GET /orders?status=Quote` return only true Quote-stage orders in production? (2) Is the staging API status field reliable? Unresolved before production go-live. |
