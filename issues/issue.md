...
| I-013 | Sprint 1 | CRITICAL | FTF API returns 207,622 orders but CRM shows 275,693 — 68,071 orders missing from API | dev_manager | OPEN | 2026-05-22 | Probed every known param (archived, deleted, all, include_archived, status=all) — total stays at 207,622. Server-side filter is baked in, likely tied to API key scope or account permissions. Also: order 1000276072 API=Quote, CRM=Checking — status field accuracy unconfirmed. FTF developer must answer: (1) Why are 68,071 orders missing? (2) Is the API key scoped to a subset? (3) Does `?status=Quote` reliably reflect CRM status? Must be resolved before Sprint 2 or classifier will miss orders. |
