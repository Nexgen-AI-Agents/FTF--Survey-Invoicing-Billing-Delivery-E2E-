...
## [2026-06-05] — Excel sheet audit: phantom rows, stale JSON, and what each FAIL actually means

- **Phantom rows (`__orphan__`, `__nesa__`)**: Leftover from the retired Teams polling system in A4. When A4 saw Teams messages it couldn't match to a real order, it stored the message IDs using those strings as the order_id key. `mark_reply_processed()` in `excel_db.py` called `save_order_state()` with no order_id validation — creating full Excel rows for non-orders. Fix: added numeric-only order_id guard in `save_order_state()` and deleted the two phantom rows.
- **`pipeline_state.json` is stale**: It's a snapshot generated by A0 at the start of each pipeline run. Any manual Excel changes (row deletion, requeue) won't appear in the JSON until the next run. Skills that need live data must read from `invoice_pipeline_state.xlsx` directly, not the JSON. The `data-quality-check` skill does this correctly.
- **Null estimate_amount for `details_missing` orders**: Expected. Those orders have no `service_type`, so A3 can't price them. Needs human data entry in FTF portal.
- **Zero estimate_amount for `data_collected` orders**: Likely from a prior run where A3 couldn't price the order, set status=`pricing_needed` + estimate_amount=0, then the order was requeued to `data_collected`. The `requeue-orders` skill clears `invoice_draft` and `data_sources` but NOT `estimate_amount`. The 0 persists. Rule: always add `estimate_amount` to the `--clear` list when requeuing for repricing.
- **`escalate_flag` not in Excel schema**: The flag is stored inside the `invoice_draft` JSON blob, not as a top-level column. `data-quality-check` will always show 0 for escalated orders until we either add the column or parse the JSON. Known limitation — document it.
