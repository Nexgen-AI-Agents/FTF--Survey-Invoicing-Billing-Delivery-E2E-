...
def send_single(order_id: str) -> None:
    ...
    msg = (
        ...
        f"\n"
        f"  <strong>Order:</strong>      {order_id}\n"
        f"  <strong>Service:</strong>    Boundary Survey\n"
        f"  <strong>Estimate:</strong>   $475.00\n"
        f"  <strong>Property:</strong>   1234 Test Lane, Orlando FL 32801\n"
        f"  <strong>FTF Link:</strong>   {link}\n"
        f"  <strong>Note:</strong>       QA test order — safe to approve or reject\n"
        f"\n"
        f"<strong>To APPROVE:</strong>   APPROVE {order_id}\n"
        f"<strong>To REJECT:</strong>    REJECT {order_id} your reason here\n"
        f"\n"
