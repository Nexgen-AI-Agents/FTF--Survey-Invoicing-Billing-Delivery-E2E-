"""
generate_client_demo_v9.py — Client demo v9

Changes from v8:
  1. Scene 10 NEW — GitHub Actions 24/7 automated monitoring (poll every 5 min)
  2. Scenes 11–15 renumbered from v8's 10–14
  3. Pipeline narration updated: "9 complete sprints", GitHub Actions mention
  4. Intro stat "Daily Auto-Remind" → "24/7 Auto-Poll"
  5. Header tag updated to "Sprint 0-9  ·  24/7 Auto-Poll"
  6. Outro updated with 24/7 GitHub Actions bullet

Output: docs/demos/YYYYMMDD_v9/demo.mp4 + transcript.md
"""
...
DEMO_DIR  = Path("docs") / "demos" / f"{DATE_TAG}_v9"
...
SCENES = [
    ...
    (1,  "problem",
     ...
     "Many new orders every week — hours of repetitive work every single day."),

    (2,  "pipeline",
     "The solution: 9 AI agents running around the clock. "
     "Detect, classify, price, approve if needed, write, review, send, report — "
     "with a smart Teams approval channel and 24/7 automated monitoring via GitHub Actions. "
     "9 complete sprints, 203 automated tests."),
    ...
    (10, "github_actions",
     "The entire approval system runs automatically — no local machine, no manual script. "
     "GitHub Actions fires the polling workflow every 5 minutes, around the clock. "
     "It reads the Teams channel, processes any APPROVE or REJECT commands within minutes, "
     "and posts confirmations back. "
     "The daily reminder fires at 9 AM weekdays automatically. "
     "Twelve secrets wired in. Zero infrastructure to maintain."),
    ...
    (15, "outro",
     ...
     "24/7 automated monitoring via GitHub Actions — no manual intervention required. "
...
SCENE_SPRINTS = {
    ...
    8:  [3],
    9:  [3],
    10: [3],              # github_actions — Sprint 3 Approval Flow
    11: [4, 5, 6],
    12: [9],
    13: [7, 8],
    14: list(range(10, 13)),
...
def _content_header(draw: ImageDraw.ImageDraw, fnt: dict, title: str,
    ...
    draw.text((20, 18), "FTF  ·  Agentic AI OS  ·  Sprint 0-9  ·  24/7 Auto-Poll", font=fnt["tag"], fill=CYAN)
    total = len(SCENES)
    dot_w, dot_r = 16, 4
    sx = w - total * dot_w - 16
...
def slide_intro(fnt: dict) -> np.ndarray:
    ...
    stats = [("9", "AI Agents"), ("78k+", "Live Invoices"), ("203", "Tests Passing"), ("24/7", "Auto-Poll")]
    ...
    tag_str = "Sprint 0-9 Complete  ·  May 2026  ·  24/7 Auto-Poll"
...
def slide_pipeline(fnt: dict) -> np.ndarray:
    ...
    draw = ImageDraw.Draw(img)
    draw.text((x0 + 24, y0 + 14), "9-AGENT PIPELINE  —  ALL BUILT & TESTED  ·  24/7 VIA GITHUB ACTIONS", font=fnt["h3"], fill=CYAN)
    ...
    rows = [
        ...
        ("Agent 4", "Human Gate",     "S3", GREEN,  "Teams alert + GitHub Actions poll every 5 min"),
...
def slide_classify(fnt: dict) -> np.ndarray:
    ...
    checks = [
        "Standard service type", "Property in Florida", "Not a competitor domain",
        "Not on NEVER_AUTO_QUOTE list", "Not in VE flood zone", "Not Monroe County",
        "Valid property coordinates", "FEMA data available",
        "No missing county", "Not out-of-state", "No competitor email detected",
...
def slide_github_actions(fnt: dict) -> np.ndarray:
    img = Image.new("RGB", (CONTENT_W, H))
    draw = ImageDraw.Draw(img)
    _content_bg(draw)
    _content_header(draw, fnt, "24/7 GitHub Actions Automation", 10)
    ...
    draw = ImageDraw.Draw(img, "RGBA")
    draw.rounded_rectangle([x0, y0, x1, y1], radius=16, fill=(8, 16, 40, 252))
    draw.rounded_rectangle([x0, y0, x1, y0+58], radius=16, fill=(12, 24, 60, 255))
    draw = ImageDraw.Draw(img)
    draw.text((x0+24, y0+16), "APPROVAL MONITOR — ZERO MANUAL INTERVENTION  ·  GITHUB ACTIONS", font=fnt["h3"], fill=CYAN)

    half_w = (x1 - x0 - 30) // 2

    # ── LEFT: Poll workflow ─────────────────────────────────────────────────
    lx = x0 + 10
    draw.rounded_rectangle([lx, y0+68, lx+half_w, y1-10], radius=10, fill=(14, 26, 56))
    draw.rectangle([lx, y0+68, lx+8, y1-10], fill=BLUE)

    draw.text((lx+18, y0+80), "poll_approval_monitor.yml", font=fnt["h3"], fill=WHITE)
    draw.text((lx+18, y0+112), "Triggers every 5 minutes — 24 hours/day, 7 days/week", font=fnt["small"], fill=GRAY)

    draw.rounded_rectangle([lx+18, y0+136, lx+360, y0+164], radius=6, fill=(16, 44, 96))
    draw.text((lx+28, y0+142), 'cron: "*/5 * * * *"', font=fnt["mono"], fill=CYAN)
    draw.text((lx+28, y0+162), 'workflow_dispatch: (manual trigger)', font=fnt["small"], fill=GRAY)

    draw.text((lx+18, y0+190), "Steps:", font=fnt["small"], fill=GRAY)
    steps = [
        ("①", "actions/checkout@v4",                      GREEN),
        ("②", "setup-python@v5  (Python 3.11)",            GREEN),
        ("③", "pip install -r requirements.txt",           GREEN),
        ("④", "poll_teams_approvals.py --since-hours 1",   CYAN),
    ]
    sy = y0 + 210
    for icon, step, col in steps:
        draw.rounded_rectangle([lx+18, sy, lx+half_w-18, sy+28], radius=4, fill=(18, 34, 72))
        draw.text((lx+26, sy+5), icon, font=fnt["small"], fill=col)
        draw.text((lx+50, sy+5), step, font=fnt["small"], fill=col)
        sy += 34

    draw.text((lx+18, sy+10), "Secrets wired in (12 total):", font=fnt["small"], fill=GRAY)
    sy += 32
    secrets_left = [
        "DATABASE_URL", "TEAMS_APP_ID", "TEAMS_CLIENT_SECRET",
        "TEAMS_CHANNEL_ID", "TEAMS_TEAM_ID", "TEAMS_TENANT_ID",
    ]
    secrets_right = [
        "FTF_API_KEY", "FTF_API_BASE_URL", "FTF_ORDER_URL",
        "APPROVED_SENDERS", "NOTIFICATION_FROM_EMAIL", "NOTIFICATION_TO_EMAILS",
    ]
    col_mid = lx + 18 + (half_w - 36) // 2
    for sl, sr in zip(secrets_left, secrets_right):
        draw.text((lx+28,    sy), "· " + sl, font=fnt["small"], fill=(100, 140, 200))
        draw.text((col_mid,  sy), "· " + sr, font=fnt["small"], fill=(100, 140, 200))
        sy += 22

    # ── RIGHT: Reminder workflow ────────────────────────────────────────────
    rx = lx + half_w + 20
    draw.rounded_rectangle([rx, y0+68, rx+half_w, y1-10], radius=10, fill=(14, 26, 56))
    draw.rectangle([rx, y0+68, rx+8, y1-10], fill=AMBER)

    draw.text((rx+18, y0+80), "approval_reminder.yml", font=fnt["h3"], fill=WHITE)
    draw.text((rx+18, y0+112), "Fires at 9:00 AM ET — Monday through Friday only", font=fnt["small"], fill=GRAY)

    draw.rounded_rectangle([rx+18, y0+136, rx+360, y0+164], radius=6, fill=(60, 40, 10))
    draw.text((rx+28, y0+142), 'cron: "0 14 * * 1-5"  (9 AM ET)', font=fnt["mono"], fill=AMBER)

    draw.text((rx+18, y0+190), "Steps:", font=fnt["small"], fill=GRAY)
    rsteps = [
        ("①", "actions/checkout@v4",                      AMBER),
        ("②", "setup-python@v5  (Python 3.11)",            AMBER),
        ("③", "pip install -r requirements.txt",           AMBER),
        ("④", "send_daily_approval_reminder.py",           CYAN),
    ]
    rsy = y0 + 210
    for icon, step, col in rsteps:
        draw.rounded_rectangle([rx+18, rsy, rx+half_w-18, rsy+28], radius=4, fill=(18, 34, 72))
        draw.text((rx+26, rsy+5), icon, font=fnt["small"], fill=col)
        draw.text((rx+50, rsy+5), step, font=fnt["small"], fill=col)
        rsy += 34

    draw.text((rx+18, rsy+10), "What it does:", font=fnt["small"], fill=GRAY)
    rsy += 32
    reminder_bullets = [
        "Finds all orders pending more than 12 hours",
        "Posts structured Teams message with FTF link per order",
        "Shows original pending-since timestamp + age badge",
        "Sorted oldest-first — most overdue at the top",
        "Repeats every weekday until all orders have a decision",
        "Same secrets pool — no extra configuration needed",
    ]
    for b in reminder_bullets:
        draw.text((rx+28, rsy), "·  " + b, font=fnt["small"], fill=(180, 150, 100))
        rsy += 26

    # ── Bottom: status bar ──────────────────────────────────────────────────
    bar_y = y1 - 50
    draw.rounded_rectangle([x0+10, bar_y, x1-10, bar_y+40], radius=8, fill=(10, 40, 20))
    draw.text((x0+24, bar_y+10),
              "24/7  ·  No server to maintain  ·  No local script to keep running  ·  Commands processed within 5 minutes",
              font=fnt["body"], fill=GREEN)
...
def slide_write_send(fnt: dict) -> np.ndarray:
    ...
    draw = ImageDraw.Draw(img)
    _content_bg(draw)
    _content_header(draw, fnt, "Write · Review · Send", 11)
    img, draw, tx, ty = _white_card(img)
...
def slide_sprint9(fnt: dict) -> np.ndarray:
    ...
    draw = ImageDraw.Draw(img)
    _content_bg(draw)
    _content_header(draw, fnt, "Sprint 9 — Memory Loop", 12)
...
def slide_ar_report(fnt: dict) -> np.ndarray:
    ...
    draw = ImageDraw.Draw(img)
    _content_bg(draw)
    _content_header(draw, fnt, "AR Report Skill", 13)
    ...
    draw.text((x0+18, cmd_y+9),
              "$ python build_ar_report.py  ->  Unpaid_AR_Report_05.28.2026.xlsx  (146 KB  ·  Sheet1: detail  Sheet2: pivot)",
...
def slide_roadmap(fnt: dict) -> np.ndarray:
    ...
    draw = ImageDraw.Draw(img)
    _content_bg(draw)
    _content_header(draw, fnt, "Sprint 10-12 Roadmap", 14)
...
def slide_outro(fnt: dict) -> np.ndarray:
    return slide_bullets(15, "Sprint 0-9: What Was Built", [
        ...
        ("24/7 automated monitoring via GitHub Actions — no manual intervention", GREEN),
...
def build_all_slides(fnt: dict) -> dict:
    ...
    s[1]  = slide_bullets(1, "The Manual Process — Before", [
               ...
               ("Many new orders every week = hours of work daily",RED),
    ...
    s[10] = slide_github_actions(fnt)
    s[11] = slide_write_send(fnt)
    s[12] = slide_sprint9(fnt)
    s[13] = slide_ar_report(fnt)
    s[14] = slide_roadmap(fnt)
    s[15] = slide_outro(fnt)
...
async def generate_all_audio() -> dict:
    ...
    for idx, slug, text in SCENES:
        ...
        print(f"    {idx:02d} {slug:<24}", end="", flush=True)
        t0 = time.time()
...
def write_transcript(audio_map: dict) -> None:
    labels = {
        ...
        10: "GitHub Actions 24/7",
        11: "Write, Review & Send",
        12: "Sprint 9 — Memory Loop",
        13: "AR Report Skill",
        14: "Sprint 10-12 Roadmap",
        15: "Summary",
    }
    lines = [
        "# FTF Agentic AI — Client Demo v9",
        f"**Date:** {datetime.now().strftime('%Y-%m-%d')}  |  **Voice:** OpenAI TTS nova (0.85x)  |  **Sprint 0-9 · 24/7 Auto-Poll**",
        "", "---", "",
    ]
    t = 0.0
...
async def build_video() -> None:
    print("\nNexGen Client Demo v9 — Sprint 0-9  ·  24/7 Auto-Poll")
    print("=" * 58)
    ...
    for idx, slug, _ in SCENES:
        ...
        print(f"  scene {idx:02d} {slug:<24} {duration:.1f}s")
    ...
    print("(3-8 minutes at 1920x1080 24fps...)\n")
